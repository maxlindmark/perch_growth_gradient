...
m1 <- gam(temp ~ area + source + s(year, by = area) + s(yday, by = area), 
          data = all_temp)
...
ggplot(data = nd, aes(yday, y = year, fill = pred, color = pred)) +
    geom_raster() +
...
# first trim the prediction dataframe with the years for which we have cohorts 
...
nd %>% 
  ...
  ggplot(aes(yday, pred, color = factor(year), linetype = source)) + 
...
nd %>% 
  ...
  group_by(area, year) %>% 
  summarise(mean_temp = mean(pred)) %>% 
  ggplot(aes(year, mean_temp, color = "prediction")) + 
  geom_line(size = 0.5, alpha = 0.5) +
  geom_line(data = errs_dat2, aes(year, mean_temp, color = "ERRS"), size = 0.5, alpha = 0.5) + 
  facet_wrap(~area) + 
  NULL
